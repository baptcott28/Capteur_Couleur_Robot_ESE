 ...
 * @return 0 if failed else 1
 */
static void color_setPhotodiodeType(h_color_sensor_t * h_color_sensor,color_sensor_color_t color);
...
/**
 * @fn void color_do_measure_again(h_color_sensor_t * h_color_sensor)
 * @brief This function is called to do a measure again after a inoperable result
 *
 * @param h_color_sensor : color_sensor structure
 * @return : Printf the color of the can
 */
static void color_do_measure_again(h_color_sensor_t * h_color_sensor);

/**
 * @fn int color_handle_calibration_values(h_color_sensor_t * h_color_sensor, uint32_t frequence,h_calib_buffer_structure_t * h_calib_struct)
 ...
 */
static int color_handle_calibration_values(h_color_sensor_t * h_color_sensor, uint32_t frequence,h_calib_buffer_structure_t * h_calib_struct);

/**
 * @fn int color_handle_raw_values(h_color_sensor_t * h_color_sensor, uint32_t frequence)
 ...
 * @return : 1 if calibration is processing, 0 if finished
 */
static int color_handle_raw_values(h_color_sensor_t * h_color_sensor, uint32_t frequence);
...
void color_sensor_init(h_color_sensor_t *h_color_sensor, color_sensor_color_t color, color_sensor_output_scale_t output_scale, color_sensor_state_t state){
	...
	h_color_sensor->green_transformation.green_coef_dir=0;
	h_color_sensor->green_transformation.green_max_freq=0;
	h_color_sensor->green_transformation.green_ord_origin=0;
	h_color_sensor->red_transformation.red_coef_dir=0;
	h_color_sensor->red_transformation.red_max_freq=0;
	h_color_sensor->red_transformation.red_ord_origin=0;
	h_color_sensor->calib_state=WAINTING_FOR_CALIB;
	color_setOutputFreqScaling(h_color_sensor);
	color_setPhotodiodeType(h_color_sensor,color);
}
...
uint32_t color_handle_calibration_sensor(h_color_sensor_t * h_color_sensor,h_calib_buffer_structure_t * h_calib_buffer_struct){
	calibration_flag=1;
	...
	while(h_color_sensor->calib_state!=CALIB_DONE){
		...
		case CALIB_VERT_CANETTE:
			color_setPhotodiodeType(h_color_sensor,GREEN);
		...
		case CALIB_ROUGE_CANETTE:
			...
			color_setPhotodiodeType(h_color_sensor,RED);
...
void color_sensor_handle_input_capture_IT(h_color_sensor_t * h_color_sensor,TIM_TypeDef *TIM,h_calib_buffer_structure_t * h_calib_buffer_struct){
	...
	if((calibration_flag==1)&&(freq<FREQ_MAX_ACCEPTABLE)){
		color_handle_calibration_values(h_color_sensor,freq,h_calib_buffer_struct);
	}
...
static int color_handle_calibration_values(h_color_sensor_t * h_color_sensor, uint32_t frequence,h_calib_buffer_structure_t * h_calib_buffer_struct){
	calib_tab[u]=freq;
	u++;
...
static void color_analyse(h_color_sensor_t * h_color_sensor, color_t green, color_t red){
	...
	if(green<red){
		...
		color_do_measure_again(h_color_sensor);
	}
	else {
		printf("c'est rouge\r\n");
	}
